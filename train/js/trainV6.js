...
function handleButtonClick(event) {
    ...
    if (buttonId in correspondances) {
		...
		if (buttonId === 'bouton_train_1') {
			console.log(type_de_case);
		}
		else if(type_de_case === Train.train2){
			console.log(type_de_case + 'caca');
		}
	}
}

function creer_train(contexte, plateau, x, y) {
	...
	else if(type_de_case === Train.train3){
		if(x-3<0){
			alert("Impossible de créer un train ici");
			return;
		}
	...
	else if(type_de_case === Train.train4){
		if(x-5<0){
			alert("Impossible de créer un train ici");
			return;
		}
...
function recuperer_case(event, contexte, plateau) {
	...
	creer_train(contexte, plateau, x, y);
